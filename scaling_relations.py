...
#       self.s = read_hdf5.snapshot(snapshot, directory=self.particle_path, dirbases=["snapdir_", ""], snapbases=["/GR_", "/gadget", "/snap_"], check_total_particle_number=True)
    ...
    def redshift(self):
        ...
        ax_main = plt.subplot(gs[0])            ax_main.set_ylabel(r'$\log_{10}(\overline{T}_{\textnormal{gas}} \, [\textnormal{keV}])$', fontsize=self.mysize, labelpad=10)
        ...
        if self.property == "T":
            ...
            ax_sub.set_ylabel(r'$\Delta \overline{T}_{\textnormal{gas}} / \overline{T}_{\textnormal{gas,GR}}$', fontsize=self.axsize, labelpad=10)
        elif self.property == "SZ":
            ax_main.set_ylabel(r'$\log_{10}(Y_{SZ} \, [Mpc^2])$', fontsize=self.mysize, labelpad=10)
            ax_sub.set_ylabel(r'$\Delta Y_{SZ} / Y_{SZ,GR}$', fontsize=self.axsize, labelpad=10)
        elif self.property == "Yx":
            ax_main.set_ylabel(r'$\log_{10}(Y_{X} \, [M_{\odot}keV])$', fontsize=self.mysize, labelpad=10)
            ax_sub.set_ylabel(r'$\Delta Y_{X} / Y_{X,GR}$', fontsize=self.axsize, labelpad=10)
        elif self.property == "Lx":
            ax_main.set_ylabel(r'$\log_{10}(L_{X} \, [keV^{1/2}M^{2}_{\odot}Mpc^{-3}])$', fontsize=self.mysize, labelpad=10)
            ax_sub.set_ylabel(r'$\Delta L_{X} / L_{X,GR}$', fontsize=self.axsize, labelpad=10)
    ...
    def z0(self, ax):
        ...
        for (mid,m) in enumerate(self.models):
            ...
            ax.set_yticklabels([r'',r'$-0.2$',r'$0.0$',r'$0.2$',r'$0.4$',r'$0.6$',r''],fontsize=self.axsize)            mass = ldmass # /0.6774
            if self.property == "T":
