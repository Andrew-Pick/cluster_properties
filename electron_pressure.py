...
#   def __init__(self, simulation, model, realisation, snapshot, system='cosma7', mass_cut=-1, delta=500, file_ending="highMass_paper", rescaling="true", core_frac=0.15):
    ...
    def calc_electron_pressure(self, group_id = -1):
        ...
        temp = gas_temperature(self.internal_energy, self.electron_abundance)  # shape N
        electron_number = self.electron_abundance * (XH * self.gas_masses / (mp / Msun))
        volumes = self.gas_masses / self.gas_densities
        electron_pressure = temp * electron_number / volumes
        positions = self.gas_positions

        print(f"Electron pressure total: {electron_pressure.shape}")

        # Save data
        df = open(pressure_dumpfile,"wb+")
        pickle.dump((electron_pressure, positions, volumes),df)
        df.close()
